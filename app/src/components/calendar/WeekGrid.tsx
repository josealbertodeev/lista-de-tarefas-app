...
export function WeekGrid({ week, selected, onSelect }: { week: Date; selected: string; onSelect: (iso: string) => void }) {
  ...
  const apptsByDay = useMemo(() => {
    const map = new Map<string, { title: string; category: Category }[]>();
    ...
    for (const occurrence of occurrencesBetween(appointments, from, to)) {
      ...
      list.push({ title: occurrence.title, category: occurrence.category });
        ...
        {days.map((day) => {
          ...
          return (
            ...
            >
              ...
              {dayAppts.length > 0 && <span className="absolute inset-x-0 top-0 h-1 bg-primary" />}

              <div className="flex items-center justify-between mb-2">
                <span className={cn('text-[10px] uppercase', isToday ? 'text-primary font-bold' : 'text-text-muted')}>
              ...
              <div className="flex flex-col gap-1">
                {[...dayAppts, ...dayTasks.map((t) => ({ title: t.title, category: t.category }))]
                  .slice(0, 3)
                  .map((item, i) => (
                    <span
                      key={i}
